...
export default function Index() {
  ...
  const effectiveSearchParams = useMemo(() => {
    ...
    return {
      ...
      prop_type: filters.propertyTypes.length > 0 ? filters.propertyTypes.join(',') : searchParams.prop_type || 'any'
    };
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke('search-listings', {
  ...
  const handleHeroSubmit = (e: React.FormEvent) => {
    ...
    if (heroInput.trim()) {
      ...
      navigate('/chats', {
        state: {
          initialMessage: heroInput.trim()
        }
      });
    }
  };
  return <div className="min-h-screen flex flex-col bg-background touch-manipulation" {...containerProps}>
      <PullToRefreshIndicator pullDistance={pullDistance} isRefreshing={isRefreshing} />
      <Navigation />

      {/* Hero Section */}
      {!hasStartedConversation ? <section className="relative min-h-[50vh] sm:min-h-[60vh] flex flex-col items-center justify-center overflow-hidden">
          <HouseHeroAnimation />
          <div className="relative z-10 text-center px-3 sm:px-4 md:px-6 pb-12 sm:pb-16 md:pb-20 max-w-5xl mx-auto">
            ...
            <form onSubmit={handleHeroSubmit} className="max-w-3xl mx-auto px-4">
              <div className="flex flex-col sm:flex-row gap-2">
                <Input value={heroInput} onChange={e => setHeroInput(e.target.value)} placeholder="Try: Find 3-bedroom fixers under $650k in Arlington with ROI over 15%" disabled={conversationLoading || searchLoading} className="h-12 sm:h-14 text-sm sm:text-base" />
                <Button type="submit" disabled={conversationLoading || searchLoading || !heroInput.trim()} size="lg" className="h-12 sm:h-14 px-6 sm:px-8 w-full sm:w-auto">
        ...
        </section> : <section className="relative py-6 sm:py-8 border-b">
      ...
      {hasStartedConversation && <div className="pb-24 sm:pb-32">
          <ConversationPanel messages={messages} loading={conversationLoading} onPropertyAnalyze={handlePropertyAnalyze} />
          ...
          {searchParams?.location && <>
              ...
              {/* Filter Panel */}
              {showFilters && <div className="max-w-7xl mx-auto px-4 mb-6">
                  <PropertyFilters filters={filters} onFiltersChange={handleFiltersChange} onClear={clearFilters} />
                </div>}

              {searchListings.length > 0 && <FeaturedHomesGrid title={`Search Results for ${searchParams.location}`} subtitle={searchData?.stale ? `Showing ${searchListings.length} cached properties (API temporarily unavailable)` : `Found ${searchListings.length} properties from ${searchData?.source || 'Zillow'}`} listings={searchListings} isLoading={searchLoading} error={searchError && searchListings.length === 0 ? "Failed to load properties" : null} hasMore={false} onAnalyze={handlePropertyAnalyze} />}
              
              {/* Loading state for property search */}
              {searchLoading && searchListings.length === 0 && <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      {hasStartedConversation && <StickyChat onSend={handleSendMessage} loading={conversationLoading || searchLoading} />}

      {/* Footer */}
      {!hasStartedConversation && <footer className="bg-muted py-8 mt-auto">
