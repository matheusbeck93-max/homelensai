...
export const VScene7Closing = ({ colors }: Props) => {
    ...
    <AbsoluteFill style={{ fontFamily, opacity: fadeIn, justifyContent: "center", alignItems: "center" }}>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 24, padding: "0 60px" }}>
        <Img src={staticFile("images/logo.png")} style={{
          width: 140, height: 140, opacity: logoOp, transform: `scale(${logoScale})`, objectFit: "contain",
        }} />
        <div style={{ opacity: textOp, transform: `translateY(${textY}px)`, textAlign: "center" }}>
          <div style={{ fontSize: 36, fontWeight: 700, color: colors.white, lineHeight: 1.4 }}>
            Big decisions deserve{"\n"}the full picture.
          </div>
        </div>
        <div style={{ opacity: tagOp, fontSize: 24, fontWeight: 700, color: colors.accent, letterSpacing: 1 }}>
