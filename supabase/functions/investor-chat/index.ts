...
async function loadInvestorChatContext(client: any, userId: string) {
  const out: {
    profile: any | null;
    savedAnalyses: any[];
    savedProperties: any[];
    ownedProperties: any[];
  } = { profile: null, savedAnalyses: [], savedProperties: [], ownedProperties: [] };
  try {
    ...
    out.profile = profileRes?.data ?? null;
    out.savedAnalyses = anRes?.data ?? [];
    out.savedProperties = savedRes?.data ?? [];
    out.ownedProperties = ownedRes?.data ?? [];
  } catch (e) {
    console.error('loadInvestorChatContext error:', e);
  }
  return out;
}

function buildInvestorContextBlock(ctx: {
  profile: any | null;
  savedAnalyses: any[];
  savedProperties: any[];
  ownedProperties: any[];
...
function buildSystemPrompt(
  ...
  investorContext?: { profile: any | null; savedAnalyses: any[]; savedProperties: any[]; ownedProperties: any[] },
) {
  const personaBlock = personaContext ? buildPersonaBlock(personaContext.persona, personaContext.secondary) : '';
  const contextBlock = investorContext ? buildInvestorContextBlock(investorContext) : '';
